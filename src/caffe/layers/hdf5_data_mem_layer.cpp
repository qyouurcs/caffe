...
HDF5MemDataLayer<Dtype>::~HDF5MemDataLayer<Dtype>() { }

// Load data and label from HDF5 filename into the class property blobs.
template <typename Dtype>
void HDF5MemDataLayer<Dtype>::LoadHDF5FileData(const char* filename) {
  if(already_loaded)
    return;
...
template <typename Dtype>
void HDF5MemDataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void HDF5MemDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int batch_size = this->layer_param_.hdf5_data_param().batch_size();
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(HDF5MemDataLayer, Forward);
#endif

INSTANTIATE_CLASS(HDF5MemDataLayer);
REGISTER_LAYER_CLASS(HDF5MemData);
