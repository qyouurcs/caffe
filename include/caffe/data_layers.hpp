...
template <typename Dtype>
class HDF5MemDataLayer : public Layer<Dtype> {
 public:
  explicit HDF5MemDataLayer(const LayerParameter& param)
      : Layer<Dtype>(param), already_loaded(false) {}
  virtual ~HDF5MemDataLayer();
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  // Data layers should be shared by multiple solvers in parallel
  virtual inline bool ShareInParallel() const { return true; }
  ...
  virtual inline const char* type() const { return "HDF5MemData"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int MinTopBlobs() const { return 1; }
